...
import { Health } from '../../../game/components/Health.js';
import { Movement } from '../../../core/components/Movement.js';
import { Renderable } from '../../../core/components/Renderable.js';
import { AI } from '../../../game/components/AI.js';
import { Collider } from '../../../core/components/Collider.js';
import { Boss } from '../../../game/components/Boss.js';
import { Experience } from '../../../game/components/Experience.js';
import { Animation } from '../../../core/components/Animation.js';
import { Weapon } from '../../../game/components/Weapon.js';
...
export class EntityFactory {
  ...
  async createEnemy(enemyId, overrides = {}) {
    ...
    if (finalConfig.ai) {
      ...
      // Add advanced AI parameters based on behavior type
      if (finalConfig.ai.patrolRadius) aiConfig.patrolRadius = finalConfig.ai.patrolRadius;
      if (finalConfig.ai.teleportCooldown) aiConfig.teleportCooldown = finalConfig.ai.teleportCooldown;
      if (finalConfig.ai.teleportRange) aiConfig.teleportRange = finalConfig.ai.teleportRange;
      if (finalConfig.ai.projectileSpeed) aiConfig.projectileSpeed = finalConfig.ai.projectileSpeed;
      if (finalConfig.ai.chargeSpeed) aiConfig.chargeSpeed = finalConfig.ai.chargeSpeed;
      if (finalConfig.ai.chargeCooldown) aiConfig.chargeCooldown = finalConfig.ai.chargeCooldown;
      if (finalConfig.ai.evadeChance) aiConfig.evadeChance = finalConfig.ai.evadeChance;
      if (finalConfig.ai.armor) aiConfig.armor = finalConfig.ai.armor;
      if (finalConfig.ai.packBonusDamage) aiConfig.packBonusDamage = finalConfig.ai.packBonusDamage;
      if (finalConfig.ai.chainRange) aiConfig.chainRange = finalConfig.ai.chainRange;
      if (finalConfig.ai.maxChains) aiConfig.maxChains = finalConfig.ai.maxChains;

      components.push(aiConfig);
    }

    // Add Animation component if animation config exists
    ...
    // Store death animation config (if defined)
    if (finalConfig.deathAnimation) {
      entity.deathAnimation = finalConfig.deathAnimation;
    }
   ...
   * @returns {Entity} Created player entity
   */
  createPlayer(config = {}) {
    const {
      x = 0,
